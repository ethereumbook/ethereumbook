...
Now that you've understood the challenges of the "first sync", we're ready to start an Ethereum client and sync the blockchain. For both geth and parity, you can use the +--help+ option to see all the configuration parameters. Other than using +--fast+ for geth as outlined in <<first_sync>>, the default settings are usually sensible and appropriate for most uses. Choose how to configure any optional parameters to suit your needs, then start geth or parity to sync the chain. Then wait...
...
MetaMask was introduced in <<intro_chapter>>, and is a versatile browser-based wallet, RPC client, and basic contract explorer. It is available on Chrome, Firefox, Opera, and Brave Browser. Find MetaMask at https://metamask.io
