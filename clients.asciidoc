...
It’s difficult to predict how fast the blockchain size would increase and when more disk space would be required, so it’s recommended to check its latest size at https://bitinfocharts.com/ethereum/.
...
* A wallet that can manage private keys and Ethereum addresses.
* The ability to create, sign and broadcast transactions.
* The ability to interact with smart contracts, using the data payload.
* A web3 library, injected into the web browser as a JavaScript object.
* The ability to access RPC services on a local or remote Ethereum node

Some lightweight clients offer, for example mobile (smartphone) wallets only basic wallet functionality. Other lightweight clients are fully-developed DApp browsers. The one common characteristic is that they offer some of the functions of a full node Ethereum client, without synchronizing a local copy of the Ethereum blockchain.
...
* An RCP client that can connect to an Ethereum full client
...
MetaMask was introduced in <<intro>>, and is a versatile browser-based wallet and basic contract explorer. It is available on Chrome, Firefox, Opera and Brave Browser. Find MetaMask at https://metamask.io
