...
==== Dapp.tools
Dapp.tools is a set of development tools who offer a comprehensive suite of blockchain-oriented developer tools in the spirit of the Unix philosophy.

https://dapp.tools/dapp/

===== Seth
Seth is a tool aimed at fitting the development process into a unix-like command-line enviorment for Ethereum.

https://dapp.tools/seth/

===== Hevm
Hevm is an implementation of the Ethereum virtual machine (EVM) made specifically for unit testing and debugging smart contracts. 
The command line program can run unit tests, interactively debug contracts while showing the Solidity source, or run arbitrary EVM code.
