...
TODO I get, obviously enough:

Compilation warnings encountered:

/home/rrt/.local/var/repo/ethereumbook/Faucet/contracts/Faucet.sol:2:1: Warning: Source file does not specify required compiler version!Consider adding "pragma solidity ^0.4.24;"
contract Faucet {
...
TODO The following lines appear in the PDF; is it because of the leading '1'? Ditto for 2_deploy_contract.js.
...
TODO There's something missing between "data" and "EmbarkJS", but I can't tell what.
...
TODO The security advice below seems a bit disingenuous, as so far you've given the reader the barest introduction to Solidity. Even supposing they are an expert developer in other languages, they are not yet in any position to evaluate Solidity code, so essentially this is just a big disclaimer. At least give some more pointers to resources for learning good Solidity practices (quite possibly there are more of those later in the book, so you could put a cross-reference here).
