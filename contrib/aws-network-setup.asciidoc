...
By default, `geth` will run for main network, and uses the folder `$HOME/ethereum/.geth` to store the downloaded blocks. To work with private nodes, `geth` needs to be configured for a new directory needs to be created - preferably under `$HOME`. Multiple such directories maybe created when working with multiple private networks.
...
[[_anchor_peering]]
=== Peering
The nodes of an Ethereum network communicate with each other to exchange transactions and newly mined blocks. For effective communication, the nodes should know about each other. This is called as _peering_ and can be acheived in three ways:
...
Note that, this set-up had not considered a detailed topology of the machines to be used for the private network. Therefore, the IP address to be used has been chosen as the public IP address. Depending on the choice of topology, it could be IP address in a private subnet, etc. 
...
----
geth --datadir "eth-priv" console // <1>
geth --datadir "eth-priv" console --rpc --rpcapi "web3,eth" --rpcaddr "0.0.0.0" --rpccorsdomain "*" consoled   // 2
...
In this section, peering of nodes does not take into account the different function each node is expected to have. To peer with nodes _and_ execute specific functions, restart peering with functions as covered in the next section.