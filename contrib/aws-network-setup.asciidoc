...
----
<1> Save the key in the folder for private network `eth-priv`.
...
1. Copy `genesis.json` (see <<_code_genesis_json>> here).
2. Start `geth` with command as shown below.

[[_code_start_with_boot_node]]
[source,bash]
----
geth --bootnodes "enode://" --datadir "eth-priv" --networkid "15" --maxpeers 3 --port 30303
----
...
[source,bash]
----
geth --datadir "eth-priv" --networkid 15 --maxpeers 3  --port 30303 console // <1>
admin.peers  // <2>
----
...
2. Copy `genesis.json` (see <<_code_genesis_json>> here).
3. Start `geth` as shown below.
...
<1> Uses `--datadir` folder for private network with network id `15` and will listen for a maximum of `3` peers in the network on UDP port `30303`. It will also launch console to add peers.
