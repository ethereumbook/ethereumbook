...
contract ExampleOraclizeContract is usingOraclize {
    ...
    event newTemperatureMeasurement(string temperature);

    function ExampleOraclizeContract() {
        update();
    }
    ...
    function __callback(bytes32 myid, string result) {
        ...
        temperature = result;
        newTemperatureMeasurement(temperature);
...
contract OracleB1IQClient {
    ...
    function OracleB1IQClient(address addr) payable {
        oracle = Oracle(addr);
        divisor = oracle.divisor();
        intraday("IBM", now);
    }

    function intraday(bytes32 ric, uint256 timestamp) {
    ...
    function handleSuccess(uint256 id) {
    ...
    function handleFailure(uint256 id) {
