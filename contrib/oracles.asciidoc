...
Thomson Reuters provide an Oracle service for Ethereum called BlockOne IQ, allowing market and reference data to be requested by smart contracts running on private or permissioned networks. A simplified version of the interface is show below. Like Oraclize, requesting contracts must be child instances:
...
contract Oracle {
    ...
    function getResponseUint(uint256 id, bytes32 name) constant returns(uint256);
