...
When AliceICO receives ether from Bob, it needs to send some AliceCoin to Bob in return. Within the AliceICO contract is a conversion rate from ether to tokens. Whatever value Alice set for this rate in creating the contract determines how many tokens can be purchased per however many ether it receives. When AliceICO calls the AliceCoin +transferFrom+ function, it sets Alice's address as the sender, Bob's address as the recipient, and uses the conversion rate to determine the "value" field which states how many tokens will be transferred to Bob. The AliceCoin contract transfers the balance from Alice's address to Bob's address and triggers a +Transfer+ event. The AliceICO contract can call +transferFrom+ an unlimited number of times, as long as it doesn't exceed the approval limit Alice set. The AliceICO contract can keep track of how many AliceCoin tokens it can sell by calling the +allowance+ function.
