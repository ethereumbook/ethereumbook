...
Non-fungible tokens are tokens that each represent a unique tangible or intangible item and therefore are not interchangeable. For example, a token that represents ownership of a _specific_ Van Gogh painting is not equivalent to another token that represents a Picasso. Similarly, a token representing a _specific_ digital collectible such as a specific CryptoKitty (see <<cryptoKitties>>) is not interchangeable with any other CryptoKitty. Therefore, non-fungible tokens will always possess a unique identifier, such as a serial number.
...
Conversely, many tokens are used to represent _extrinsic_ things, like real-estate, corporate voting shares, trademarks, gold bars. The ownership of these items, which are not "within" the blockchain, is governed by law, custom and policy that are separate from the consensus rules that govern the token. In toher words, token issuers and users may still rely on real non-smart contracts. As a result, these extrinsic assets carry additional counterparty risk because they are held by custodians, recorded in external registries, or controlled by laws and policies outside the blockchain environment. As of this moment, regulation and consumer protection are hot topics in many countries.
...
Nevertheless, some of the most innovative business ideas are indeed taking place in the crypto realm. If regulators are not quick enough to adapt laws to support new business models, talents and entrepreneurs will seek to operate in other jurisdictions which are more crypto-firendly. This is happening right now.
...
One fo the benefits of implementing services on top of general-purpose frameworks like Ethereum is exactly to facilitate the integration or connection between smart contracts, thus increasing the potential for liquidity and utility of tokens.
...
//I believe it's natural and expected that when first exposed to the token concept, readers will assume that tokens are owned and transferred between Ethereum accounts (addresses) just like ether. Meaning the users will be able to transfer tokens with simple native transactions, send tokens to any address or receive tokens from any other user without using a smart contract. Personally, this has been a source of confusion for me until I got to study contract codes. Should we introduce this topic here before presenting standards, functions and events? Or this will actually complicate matters?
...
//Probably the first time "ICO" is mentioned in the book. Link to a reference?
...
https://github.com/ethereumbook/ethereumbook/blob/develop/code/truffle/Faucet/truffle-config.js
