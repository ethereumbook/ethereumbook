...
Advantages: Adoption of a large number of already available distributed network of mining nodes.
Disadvantages: Inefficient energy consumption. Centralized hashing power with concentrated mining farms instead of being truly distributed. Massive amount of computing power required to mine new blocks and its impact on the environment. Mining nodes are unknown.

Proof-of-Authority is a protocol that distributes the mining load only to authorized and trusted signers that may mint new blocks at their own discretion and at any time with a mining frequency. https://github.com/ethereum/EIPs/issues/225
...
Disadvantages: The definition and application of the selection algorithm for approved accounts.
