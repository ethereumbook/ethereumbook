...
* Has more than 2,000 reviews
* Has more than 10,000,000 users
...
((("MetaMask","wallet setup with", id="ix_02intro-asciidoc4", range="startofrange")))Once MetaMask is installed you should see a new icon (the head of a fox) in your browser's toolbar. It's useful to "pin" the Metamask extension to the toolbar, if it's not already, by clicking on the Puzzle piece icon and then the pin icon as shown in <<metamask_pin>>.

[[metamask_pin]]
.Pinning the Metamask extension to the Chrome toolbar
image::images/metamask_pin.png["Pinning MetaMask extension"]

Click on the **Metamask icon** to automatically open a new browser tab. Click **Get Started** and follow the prompts. Assuming this is the first time you're using an Ethereum wallet, click on **Create a Wallet** as shown in <<metamask_create_wallet>>.

[[metamask_create_wallet]]
.Creating a new Metamask Wallet
image::images/metamask_create_wallet.png["Creating a new Metamask Wallet"]

Next, you will be asked to optionally send usage data to help improve Metamask. This is your personal preference. 

Afterwards, you will be asked to create your new Ethereum wallet by entering a **password** (see <<metamask_password>>) and accepting the **Terms of Use**.
...
[TIP]
====
This **password** controls access to MetaMask, so that it can't be used by anyone with access to your browser.
====
...
Your account page shows the name of your account ("Account 1" by default), an Ethereum address (redacted in the example), and a colorful icon to help you visually distinguish this account from other accounts. At the top of the account page, you can see which Ethereum network you are currently working on ("Ethereum Mainnet" in the example).
...
((("Ropsten Test Network")))Enable the test networks within Metamask, if you have not already, then switch MetaMask to the _Ropsten Test Network_. Within Metamask click **Buy**, then click *Get Ether* from the Test Faucet as shown in <<metamask_get_ether>>. 

[[metamask_get_ether]]
.Get Ether from MetaMask Test Faucet
image::images/metamask_get_ether.png["Get Ether from MetaMask Test Faucet"]

MetaMask will open a new web page, as shown in <<metamask_ropsten_faucet>>.

[[metamask_ropsten_faucet]]
.MetaMask Ropsten Test Faucet
image::images/metamask_ropsten_faucet.png["MetaMask Ropsten Test Faucet"]

You may notice that the web page already contains your MetaMask wallet's Ethereum address. If it does not, click on the **request 1 ether from faucet** button and **Connect** your account to the web page. 

MetaMask integrates Ethereum-enabled web pages with your MetaMask wallet and can "see" Ethereum addresses on the web page, allowing you, for example, to send a payment to an online shop displaying an Ethereum address. MetaMask can also populate the web page with your own wallet's address as a recipient address if the web page requests it. In this page, the faucet application is asking MetaMask for a wallet address to send test ether to.
...
When you first load Remix, it will start with a tab containing a Home screen with Quicklinks and other shortcuts. Since we don't need this right now, close the tab by clicking on the +X+, as seen in <<remix_close_tab>>. For reference, you can re-open this tab by clicking the Remix icon towards the top left.

[[remix_close_tab]]
.Close the default Home tab
image::images/remix_close_tab.png["Close the default Home tab"]


Next, add a new file and name it `Faucet.sol` by clicking on the Create New File button in the top-left toolbar, as shown in <<remix_toolbar>>.

[[remix_toolbar]]
.Click the Create New File button
image::images/remix_toolbar.png["Click the Create New File button"]


With the new tab open, copy and paste the code from our example `Faucet.sol`, as shown in <<remix_faucet_load>>.
...
Now we need to compile the Solidity code into bytecode. Open the **Solidity compiler** tab in the left sidebar and click **Compile Faucet.sol**, as shown in <<remix_compile_faucet_button>>.

[[remix_compile_faucet_button]]
.Click the Compile Faucet.sol button
image::images/remix_compile_faucet_button.png["Click the Compile Faucet.sol button"]


Because newer versions of Solidity have been released since the writing of this contract, Remix may display an error due to different compiler versions, as shown in <<remix_solidity_compiler_error>>

[[remix_solidity_compiler_error]]
.Remix displaying a compiler error
image::images/remix_solidity_compiler_error.png["Copy the Faucet example code into the new tab"]


In this case, our `pragma` directive prevents `Faucet.sol` from compiling, because the version it references (`0.6.4`) is different from the version Remix is currently using. To change the compiler version, go to the **Solidity compiler** tab, set the version to `0.6.4`, and try again.

Once the Solidity compiler has compiled our `Faucet.sol` successfully into EVM bytecode, it should display a green checkmark and show **Faucet (Faucet.sol)** in the Contract dropdown list as shown in <<remix_faucet_compiled>>. 

[[remix_faucet_compiled]]
.Remix compiled Faucet contract successfully
image::images/remix_faucet_compiled.png["Remix compiled Faucet contract successfully"]
