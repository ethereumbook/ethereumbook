...
When you first load Remix, add a new file, as seen in <<remix_toolbar>>. Name the new file _Faucet.sol_.

[[remix_toolbar]]
.Click to add a new file
image::images/remix_toolbar.png["Click to add a new file"]

Once you have the new file tab open, copy and paste the code from our example _Faucet.sol_, as seen in <<remix_faucet_load>>.
...
Once you have loaded the _Faucet.sol_ contract into the Remix IDE, go to the Compile tab, to ensure the Remix IDE is using a version of the Solidity compiler 0.6.0, as our pragma directive in the code will prevent _Faucet.sol_ from compiling otherwise. (see <<remix_compile>>).
