...
We now see a new transaction with the contract address as the destination and zero ether. The contract balance has changed and is now 0.9 ether, because it sent us 0.1 ether as requested. But we don't see an "OUT" transation in the contract address history.
