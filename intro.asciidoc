...
Once you have confirmed that you have stored the mnemonic securely, you'll be able to see the details of <<metamask_account>>:
...
TODO Move "The issue…" to a footnote? It's introduced abruptly and confusingly, and is mostly of interest to readers who already know something. The sentence could start differently too, e.g. "In 2017 [or whenever it was] Ropsten was attacked, so that it became useless for a while: the attacker…" and delete "also".
...
Fortunately, we are not test ether hoarders; let's practice <<send_to_faucet>>.
...
Select "View Account on Etherscan", to open a web page in the block explorer, showing your account's <<block_explorer_account_history>>:
...
TODO: The first line does not currently match the first line of the code. Also the URL appears to be wrong (does not include +/Solidity+ after +/code+).
...
When you first load Remix, it will start with a sample contract called +ballot.sol+. We don't need that, so let's close it, clicking on the +x+ on the corner of the tab, as seen in the image <<remix_close_tab>>:

[[remix_close_tab]]
.Close the default example tab
image::images/remix_close_tab.png["Close the default example tab"]

Now, add a new tab by clicking on the circular-plus-sign in the left toolbar, naming the new file +Faucet.sol+, as seen in the image <<remix_toolbar>>

[[remix_toolbar]]
.Click the plus sign to open a new tab
image::images/remix_toolbar.png["Click the plus sign to open a new tab"]

Once you have a new tab open, copy and paste the code from our example +Faucet.sol+, as seen in the image <<remix_faucet_load>>:
...
Now we have loaded the +Faucet.sol+ contract into the Remix IDE, the IDE will automatically compile the code. If all goes well, you will see a green box with "Faucet" in it appear on the right, under the Compile tab, confirming the successful compilation, as you'll see in the image <<remix_compile>>
...
Remix will construct the special "creation" transaction and MetaMask will ask you to approve it. You'll see it in <<remix_metamask_create>>. You'll notice the contract creation transaction has no ether in it, but it has 258 bytes (the compiled contract) and will consume 10 Gwei in gas. Click "Submit" to approve it:
...
Once the contract is created, it appears at the bottom of the Run tab and you'll see <<remix_contract_interact>>:
...
Keep Remix open; we'll come back to it again later. Now, navigate your browser to +ropsten.etherscan.io+ and paste the address into the search box. You can <<etherscan_contract_address>>. You should see the contract's Ethereum address history:
...
Our faucet needs funds!Our first project will be to: <<metamask_send_to_contract>>. You should still have the address of the contract in your clipboard (if not, copy it again from Remix). Open MetaMask, and send 1 ether to it, exactly as you would any other Ethereum address:
