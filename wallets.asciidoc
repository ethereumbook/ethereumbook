...
TODO Explain why creating a new address for each transaction is expensive if you deal a lot with tokens.
...
[TIP]
====
Ethereum wallets contain keys, not ether or tokens. Wallets are like keychains containing pairs of private and public keys (see <<private_public_keys>>). Users sign transactions with the private keys, thereby proving they own the ether. The ether is stored on the blockchain.
====
...
TODO: In the image, "11 bits" and "132 bits" should not be hyphenated.
