...
((("code examples, obtaining and using", id="codeuse00")))The examples are illustrated in Solidity, JavaScript and Python, and using the command line of a Unix-like operating system. All code snippets are available in the GitHub repository in the https://github.com/ethereumbook/ethereumbook/tree/first_edition/code[_code_] subdirectory of the main repo. Fork the book code, try the code examples, or submit corrections via GitHub:
