...
The temporary key pair is based on a random number _k~eph~_, which is used as the temporary private key. From _k~eph~_, we generate the corresponding temporary public key _P_ (calculated as _P = k~eph~*G_, in the same way Ethereum public keys are derived; see <<pubkey>>). The _R_ value of the digital signature is then the x coordinate of the ephemeral public key _P_.

From there, the algorithm calculates the _S_ value of the signature, such that:

_S_ ≡ __k~eph~__^-1^ (__Keccak256__(__m__) + __k~priv~__ * __R__)  {nbsp} {nbsp} (_mod p_)

where:

* _k~eph~_ is the ephemeral private key
* _R_ is the x coordinate of the ephemeral public key
* _k~priv~_ is the signing (EOA owner's) private key
...
_P_ ≡ __S__^-1^ * __Keccak256__(__m__) * _G_ + __S__^-1^ * _R_ * _K~pub~_  {nbsp} {nbsp} (_mod p_)

where:

* _R_ and _S_ are the signature values
* _K~pub~_ is the signer's (EOA owner's) public key
