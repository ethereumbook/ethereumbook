...
nonce is also used to prevent double spending, by checking if the nonce is not set to the same as another transaction it will be avoided as the previous transaction has not been confirmed yet and hence the nonce is invalid, if the nonce is set to the same only one of the transactions will be mined, no double spend will happen as only one transaction will take place.
