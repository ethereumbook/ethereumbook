...
The transaction message's structure is serialized using the Recursive Length Prefix (RLP) encoding scheme (see <<rlp>>), which was created specifically for simple, byte-perfect data serialization in Ethereum. All numbers in Ethereum are encoded as big-endian integers, of lengths that are multiples of 8 bits.
...
TODO Explain how gas being a separate currency from ether allows the ratios of the costs between the various resources to be managed. (That would seem to be a separate problem that involves changing their relative prices, which could as easily be done in ether as in gas.)
...
TODO: "special price": do you mean "special prize"? I'm not sure I really understand either way.
...
When you construct an Ethereum transaction that contains value, it is the equivalent of a _payment_. Such transactions behave differently depending on whether the destination address is a contract or not.
...
Here we can see the address of the contract. We can send funds to and receive funds from the contract as shown in <<data_EOA>>.
...
((("transactions", "digital signatures and")))So far, we have not delved into any detail about digital signatures. In this section, we look at how digital signatures work and how they can be used to present proof of ownership of a private key without revealing that private key.
