...
++++
<pre data-type="programlist">
$ <strong>npm install openzeppelin-solidity@2.0.0</strong>

+ openzeppelin-solidity@2.0.0
added 1 package from 1 contributor and audited 1 package in 0.612s
...
++++
<pre data-type="programlist">
$ <strong>truffle migrate --network dev</strong>
Using network 'dev'.
...
++++
<pre data-type="programlist">
$ <strong>truffle console --network dev</strong>
truffle(dev)&gt;
</pre>
++++

The +truffle(dev)>+ prompt shows that we are connected to the +ganache+ blockchain and are ready to type our commands. The Truffle console supports all the +truffle+ commands, so we could +compile+ and +migrate+ from the console. We've already run those commands, so let's go directly to the contract itself. The +METoken+ contract exists as a JavaScript object within the Truffle environment. Type ++**METoken**++ at the prompt and it will dump the entire contract definition:

++++
<pre data-type="programlist">
truffle(dev)&gt; <strong>METoken</strong>
...
++++
<pre data-type="programlist">
truffle(dev)&gt; <strong>METoken.address</strong>
...
++++
<pre data-type="programlist">
truffle(dev)&gt; <strong>METoken.deployed().then(instance => instance.totalSupply())</strong>
...
++++
<pre data-type="programlist">
truffle(dev)&gt; <strong>let accounts</strong>
undefined
truffle(dev)&gt; <strong>web3.eth.getAccounts((err,res) => { accounts = res })</strong>
undefined
truffle(dev)&gt; <strong>accounts[0]</strong>
...
++++
<pre data-type="programlist">
truffle(dev)&gt; <strong>METoken.deployed().then(instance =></strong>
                  <strong>{ instance.balanceOf(accounts[0]).then(console.log) })</strong>
undefined
truffle(dev)&gt; <strong>BigNumber { s: 1, e: 9, c: [ 2100000000 ] }</strong>
</pre>
++++
...
++++
<pre data-type="programlist">
truffle(dev)&gt; <strong>METoken.deployed().then(instance => 
                  { instance.transfer(accounts[1], 100000) })</strong>
undefined
truffle(dev)&gt; <strong>METoken.deployed().then(instance => 
                  { instance.balanceOf(accounts[0]).then(console.log) })</strong>
undefined
truffle(dev)&gt; <strong>BigNumber { s: 1, e: 9, c: [ 2099900000 ] }</strong>
undefined
truffle(dev)&gt; <strong>METoken.deployed().then(instance => 
                  { instance.balanceOf(accounts[1]).then(console.log) })</strong>
undefined
truffle(dev)&gt; <strong>BigNumber { s: 1, e: 5, c: [ 100000 ] }</strong>
</pre>
++++
...
<pre data-type="programlist">
$ <strong>truffle console --network dev</strong>
truffle(dev)&gt; <strong>compile</strong>
Compiling ./contracts/Faucet.sol...
Writing artifacts to ./build/contracts

truffle(dev)&gt; <strong>migrate</strong>
...
truffle(dev)&gt; <strong>BigNumber { s: 1, e: 0, c: [ 0 ] }</strong>
truffle(dev)&gt; <strong>METFaucet.deployed().then(instance =>
                  { instance.withdraw(1000, {from:web3.eth.accounts[1]}) } )</strong>
truffle(dev)&gt; <strong>METoken.deployed().then(instance =>
                  { instance.balanceOf(web3.eth.accounts[1]).then(console.log) })</strong>
truffle(dev)&gt; <strong>BigNumber { s: 1, e: 3, c: [ 1000 ] }</strong>
</pre>
++++
