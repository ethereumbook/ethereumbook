...
This formal specification, in addition to various Ethereum Improvement Proposals, define the standard behavior of an Ethereum client.
...
From Parity segway into browsers. What is it? Ok, so Parity has one. Are there others?
...
=== Frameworks
Frameworks can be used to ease development. By doing everything yourself you get a better understanding of how everything fits together, but it's a lot of teidious work and it's mostly doing the same over and over. These frameworks can automate some of these tasks and make development a breeze.
