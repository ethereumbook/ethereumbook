...
((("Yellow Paper specification")))Ethereum is defined by a formal specification called the "Yellow Paper" (see link:01what-is.asciidoc[references]).
...
The full JSON-RPC API can be investigated on the https://ethereum.github.io/execution-apis/api-documentation/[Ethereum wiki].
...
https://metamask.io/[MetaMask], ((("MetaMask","as browser wallet")))introduced in link:02intro.asciidoc#choosing_eth_wallet[intro_chapter], is a versatile browser-based wallet, RPC client, and basic contract explorer. It is available on Chrome, Firefox, Opera, and Brave Browser.
