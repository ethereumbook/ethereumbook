...
In 2015 an attacker exploited an EVM instruction that costed far less gas than it should have. this allowed the attacker to create transactions that use a lot of memory and take several minutes to validate. To fix this attack, Ethereum had to change its gas accounting formula for certain instructions in a backwards incompatible (hard fork) change. Even with this change, however, Ethereum clients have to skip validating these transactions or waste weeks trying to validate them.
...
Eventually, Ethereum will slow down and its interfaces will become fixed. But in the mean time, innovation is the driving principle. You'd better keep up, because no one will slow down for you.

=== Why learn Ethereum

Ethereum is a great platform to learn about blockchains. More than any other blockchain, Ethereum is a _developer's blockchain_, build by developers, for developers. A developer familiar with JavaScript applications can drop into Ethereum and start producing working code very quickly. For the first years of Ethereum, it was common to see t-shirts announcing that you can create a token in just five lines of code. Of course, this is a double-edged sword. It's easy to write code, but it's very hard to write _good_ code and _secure_ code. When your code handles money, the results can make for some spectacular headlines.

Nevertheless, with time comes maturity and Ethereum is building a massive community of developers, faster than any other blockchain platform.
