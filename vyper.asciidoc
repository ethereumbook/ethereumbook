...
:revnumber: v1.4
:revdate: 2018-04-24 (yyyy-mm-dd)
:revremark: TODO - The Inline assembly, Function overloading and Variable typecasting sections listed below have to be re-worded. They are, by and large, just a cut and paste of the original documentation and also contain TODOs
...
Class inheritance:: Inheritance allows programmers to harness pre-written code by acquiring pre-existing functionality, properties and behaviors from existing software libraries. Inheritance is powerful and promotes the reuse of code. However, it also creates an issue for those who write and audit code. Inheritance forces programmers and code auditors, who seek to understand precisely what a program is doing, to jump between multiple files while storing complex information in their memory. It can be argued that, given the immutability of blockchains, leveraging powerful, obscure and sometimes unintelligible software libraries can wreak havoc in smart contract implementation. For example a programmer inadvertently triggering pre-written code in an incorrectly coded smart contract.
