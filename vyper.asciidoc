...
Implicit typecasting is often performed at compile time. For example if a type conversion is semantically sound and no information is likely to be lost the compiler could perform an implicit conversion, such as converting a variable of type uint8 to uint16.

Explicit type conversion can be performed by a software developer in the source code. Unfortunately, explicit type conversions in code can often lead to unexpected behaviour. For example, if an explicit type conversion transposes a uint32 type to the smaller type of uint16, the higher-order bits are simply cut off as demonstrated below. 
...
Whilst the earliest versions of Vyper entertained typecasting of variables, the most recent version of the Vyper programming language does not support variable typecasting.
