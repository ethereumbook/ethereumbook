...
A Vyper smart contract can only consist of a single Vyper file. In other words, all of the smart contract's code, including all functions, variables and so forth exist in one place. Vyper also requires that functions and variable declarations are physically written in a particular order, in the file. Solidity does not have this requirement at all. Let's take a quick look at a Solidity example.
...
Vyper's ordering requirements are not a new thing, ordering like this is something which has always been a requirement in Python programming. The ordering, required by Vyper, is straight forward and logical as we will see in this next example.
