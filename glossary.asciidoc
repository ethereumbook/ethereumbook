...
gas::
    ...
    Gas is an unit of computation that is incurred per instruction of Smart Contract executed. The gas is pegged at Ether crypto-currency. The gas is analogous to talk time on a cellular network. Thus, the price of running a transaction in fiat currency is `gas * (ETH/gas) * (fiat currency/ETH)`.
...
hash::
    A digital fingerprint of some input calculated using cryptographic algorithm such as SHA, etc.
