...
big-endian::
    This type of Endianness the most significant unit is stored first. The unit referred here is generally a bit or half of a byte in its hexadecimal representation as per context.
...
Recursive Length Prefix (RLP):: RLP is a encoding standard, designed by the Ethereum developers to encode and serialize objects (data structures) of arbitrary complexity and length.
