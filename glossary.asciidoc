...
Deed::
  Non-fungible token (NFT) standard introduced in ERC721 proposal. Unlike ERC20 tokens, deeds prove ownership and are not interchangeable, though they are not recognized as legal documents in any jurisdiction, at least not currently (also see the definition for NFT, below).
...
Keccak256::
  Cryptographic hash function used in Ethereum. Keccak256 was standardized to SHA-3.
...
NFT::
    Non-fungible tokens (also known as deeds). This is a token standard introduced in the ERC721 proposal. NFTs can be tracked and traded, but each token is unique and distinct, they are not interchangeable like ERC20 tokens. NFTs can represent ownership over digital or physical assets (also see the definition for deeds, above).
...
Swarm::
  A decentralized (P2P) storage network. It is used along with Web3 and Whisper to build DApps.
