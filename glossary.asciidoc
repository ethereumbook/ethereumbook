...
Inmutable Deployed Code Problem::
  Once contract's (or library's) code is deployed it become immutable. Being able to fix possible bugs and add new features is key for the software development cycle.
  Thus, we’d need some mechanism to upgrade the contract’s (or library's) code.

internal transaction (also "message")::
    A transaction sent from a contract account to another contract account or an EOA.

library (Solidity -> EVM)::
  A library in Solidity ( -> EVM ) is a special type of contract without payable functions neither a fallback function.
  Therefore, it cannot hold ether. Furthermore, a library has not any kind of storage.
  You could think of a library as a previously deployed code that can be called from any contract without the need to deploy it again.
