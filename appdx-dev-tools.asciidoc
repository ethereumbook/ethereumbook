...
We're all set. Let's use +truffle migrate+ to deploy it. We have to specify which network to deploy the contract, using the +--network+ argument. We only have one network specified in the configuration file, which we named +localnode+. Make sure your local Ethereum client is running and then type:
