...
Without a node infrastructure provider like Alchemy, Ethereum developers need to spin up their own self-hosted nodes, which can present a number of challenges. Running your own node can be time consuming and expensive, requiring significant engineering time to manage and ~$7k in AWS server costs per node every year. On top of that, these nodes can be unreliable - running into issues with CPU spikes, peering failures, and consistency issues when nodes fall out of sync with the network. 
