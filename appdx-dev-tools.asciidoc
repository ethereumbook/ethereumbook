...
Alchemy API is a node infrastructure provider that maintains nodes as cloud services and executes node requests for you. It serves access to regularly updated Geth and Parity nodes that guarantee reliability, data correctness and scalability.

It also provides additional developer tools such as:

==== API Explorer
Browser tool allowing you to search and analyze all your API requests to track patterns, find errors, and simplify debugging.

==== Mempool Visualizer
Allows you to view and search for mined, pending, and dropped transactions in an Alchemy-hosted mempool, so you can track the status of your transactions.

==== Composer
Allows you to create test API requests using a simplified online UX instead of the command line.

==== Notify
Uses Webhooks to allow you access to notifications whenever you see activity on a particular Ethereum address, have transactions in your mempool mined or dropped, or when Ethereum gas prices drop below a certain amount.
