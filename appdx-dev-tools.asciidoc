...
Alchemy API is a Ethereum infrastructure platform and node provider that maintains nodes as cloud services and executes node requests on your behalf. It provides access to managed Geth and Parity nodes, a set of developer tools that simplify Ethereum development, and tools to monitor and notify you on the status of your transactions.

Without a node infrastructure provider like Alchemy, Ethereum developers need to spin up their own self-hosted nodes, which can cost an average of $86k a year accounting for AWS costs and engineering time. These nodes are time consuming, requiring months of engineering time to develop robust infrastructure, and failures can take up to 3 weeks to recover. On top of that, these nodes can be extremely unreliable - running into issues with CPU spikes, memory leaks, peering failures, corrupted databases, and consistency issues when nodes fall out of sync with the network. 

Alchemy's primary tool is a proprietary infrastructure engine that encapsulates away the complexities of managing Ethereum nodes using a JSON-RPC request interface that extends `web3.js`. By routing node requests to a custom API endpoint, developers can get access to everything necessary to communicate with the Ethereum blockchain while guaranteeing that their requests are served reliably, with full data correctness, and can be scaled to any size necessary.

Alchemy also provides additional developer tools such as: 
...
image::images/alchemy-new-app.gif["Parity asking for confirmation to deploy Faucet"]
...
((("Truffle","configuring")))+Truffle+ creates an empty configuration file, _truffle-config.js_. Now we edit _truffle-config.js_ and replace the contents with the sample configuation shown here:
