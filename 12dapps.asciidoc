...
((("Swarm","installing and initializing")))To get started, you need to install Swarm and initialize your Swarm node. Swarm is part of the Ethereum Foundation's Go-Ethereum suite of tools. Refer to the instructions for installing Go-Ethereum in https://github.com/ethereumbook/ethereumbook/blob/develop/03clients.asciidoc#software-requirements-for-building-and-running-a-client-node[go_ethereum_geth], or to install a Swarm binary release, follow the instructions in the http://bit.ly/2Q75KXw[Swarm documentation].
